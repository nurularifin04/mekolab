...
import { show, hide, bytesToText, attributeAssignment, createCanvas, updateGrid, SourSwal, Code } from "./utils.js";
...
function jsqrDecode(image) {
  ...
  const [cvs, ctx] = createCanvas(width, height, 'transparent');
  ...
  if (!code) {
    ...
    ui.image.canvas.replaceChildren(cvs);
    return code;
  }
}
...
function displayBlocks() {
  ...
  sortedBlockType.forEach(block => {
    ...
    card.dataset.count = `${block.count}`; /* use to css class => .block-card::after */
...
function displayProperties() {
  ui.properties.header.textContent = `
    [${String(selectedId).padStart(3, '0')}] ${blockDefinitions[selectedId].name}
  `;
